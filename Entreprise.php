  ...
  private $_numero;
   ...
   function __construct($nom,$prenom,$email,$mdp) {
	 ...
	 function add(){
		...
		$sql = $conn->prepare("INSERT INTO Entreprise (nom,mdp,email,prenom) VALUES (?, ?, ?, ?)"); 
		...
		$conn->close();
	 	 ...
	 	 function login(){
			require_once("connectDB.php");
				$stmt = $conn->prepare("SELECT id,nom , prenom , email FROM Entreprise WHERE email = ? and mdp = ?") ;
	 ...
	 function update(){
		...
		require_once("connectDB.php");
		 @session_start();
			  $stmt = $conn->prepare("UPDATE Entreprise SET nom = ?, 
									   prenom= ?, 
									   email = ?,  
									   mdp= ? where 
									   id = ?") ;
